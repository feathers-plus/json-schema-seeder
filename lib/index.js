...
module.exports = function (options1 = {}) {
    ...
    Object.keys(jsonSchemas).forEach(tableName => {
      ...
      if (!options1.testNoKeyConvert) {
        ...
        schema.properties[adapter.idName] = {
          type: 'string', // faker property requires 'string'
  ...
  function normalizeForJsf (fieldSchema, fieldName, schema) {
    if (
    ...
    // Remove ID field if it lacks a faker.fk prop as we don't know if its integer or string.
    if (fieldSchema.type === 'ID') {
      delete schema.properties[fieldName];
      debug('delete fieldName', fieldName);
    }
  }
};
...
function traverseJsonSchema (schema = {}, cb) {
  ...
  Object.keys(properties).forEach(fieldName => {
    ...
    cb(fieldSchema, fieldName, schema);
